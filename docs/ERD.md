...
| LogModal.tsx       | Custom Points Picker / Slider   | KindnessEntry.pointsAwarded | z.number().int().min(5).max(100)                                       | Automatically resolved on the server for standard categories; accepts custom variables for 'Other'.            |

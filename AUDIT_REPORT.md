...
| **Low**       | `.env.example`                                                                      | `ADMIN_PIN=1234` is the literal example value — if a deployer copies `.env.example` to `.env` without editing it, the PIN is a famously weak default.                                                                                                                             | Add a guard (similar to the existing `JWT_SECRET` check in `getAdminPin()`) that throws if `ADMIN_PIN` is unset _or_ still `1234`. **✅ Applied** — `packages/backend/src/middleware/auth.ts`. Note: this fires on the first login attempt, not at server boot, since `getAdminPin()` is called inside the request handler rather than at module load (same lazy-check pattern as the pre-existing `JWT_SECRET` guard) — verified it fails safe (masked 500, server stays up) rather than crashing the process. |
...
### Ready-to-apply fix: reject the default PIN on first login attempt

`getAdminPin()` is called inside the login request handler, not at module load — so this guard fires the first time someone hits `/api/auth/login`, not literally when the server boots. Confirmed: with `ADMIN_PIN=1234` set, a login attempt returns a masked `500` and the rest of the server (e.g. `/api/health`) keeps responding normally — it fails safe without crashing the process, it just isn't a true startup check.
